...
public class EventItemHelper {
	...
	public List<EventItem> searchForEventByType(String eventType) {
		...
		EntityManager em = emfactory.createEntityManager();
		em.getTransaction().begin();
		TypedQuery<EventItem> typedQuery = em
				.createQuery("select ei from EventItem ei where ei.eventType = :selectedEventType", EventItem.class);
	...
	public List<EventItem> searchForEventByName(String eventName) {
		...
		em.getTransaction().begin();
		TypedQuery<EventItem> typedQuery = em
				.createQuery("select ei from EventItem ei where ei.eventName = :selectedEventName", EventItem.class);
	...
	public List<EventItem> searchForEventByDate(LocalDate eventDate) {
		...
		em.getTransaction().begin();
		TypedQuery<EventItem> typedQuery = em
				.createQuery("select ei from EventItem ei where ei.eventDate = :selectedEventDate", EventItem.class);
